...
public class SkipList {
    ...
    public void add(int value) {
        ...
        //基于上述结果拿到当前节点的后继节点
        Node p = h;// 头节点存储了所有所有层的起始位置
        for (int i = level - 1; i >= 0; i--) {//从当前节点的最高层开始找，但这有个问题，如果level=1.可能要找挺久的？
            while (p.forwards[i] != null && p.forwards[i].data < value) {
                p = p.forwards[i]; // p定位到下一个节点
            }
            maxOfMinArr[i] = p;// 存储了第i层，之前第一个 大于等于value值的节点（说明在这一层，当前节点应该在p节点后面） 或者是 第i层最后一个节点
        }

        //更新前驱节点的后继节点为当前节点newNode
        for (int i = 0; i < level; i++) {
            newNode.forwards[i] = maxOfMinArr[i].forwards[i];// 因为有哨兵节点，所以不需要考虑边界情况
    ...
    public void delete(int value) {
        Node p = h;
        //找到各级节点小于value的最大值。第一个小于value的索引节点
